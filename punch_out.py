...
while True:
    ...
    for face in faces:
        ...
        # Punch-Out once
        if name != "Unknown" and name not in punched_out:
            time = datetime.now().strftime("%H:%M:%S")
            pd.DataFrame([[name, time, "Punch-Out"]],
                         columns=["Name","Time","Type"])\
                .to_csv("attendance.csv", mode="a", header=False, index=False)
            punched_out.add(name)
            print(f"{name} punched out at {time}")
    ...
    cv2.imshow("Punch-Out Camera", frame)
